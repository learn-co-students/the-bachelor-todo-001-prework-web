...
def get_first_name_of_season_winner(data, season)
  data[season].each do |contestants| #second argument season is the potential key for data hash
...
def get_contestant_name(data, occupation)
  data.each do |season, contestants|
     contestants.collect do |contestant_data|
...
def count_contestants_by_hometown(data, hometown)
  counter = 0
  data.each do |season, contestants|
    contestants.each do |lady_data|
      if lady_data["hometown"] == hometown
        counter += 1
      end
    end
  end
...
def get_average_age_for_season(data, season)
  age_total = 0
  num_of_cons = 0
    data[season].each do |con_data|
      age_total += (con_data["age"]).to_i
      num_of_cons += 1
    end
    (age_total / num_of_cons.to_f).round(0)
end
